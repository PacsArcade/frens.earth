...
6. **Keys never leave the signer.** Not into forms, not into files, not onto
   servers. The wallet lives on the node; the fren's key lives in their
   extension.
7. **The server verifies, never vouches.** Signatures are the authority —
   sessions are convenience. High-stakes actions take per-action signatures.
8. **Every identity claim must be checkable by anyone** (registry, NIP-05,
   signed attestations — never "trust us").

## Ops

9. **Env vars need a redeploy; deploys are CLI-pushed.** (Learned the hard
   way, kept forever.)
10. **`data/*.json` is never tracked** — dev stores only; some hold tokens.
11. **Ship's log rides every significant push** — the repo is the record.
